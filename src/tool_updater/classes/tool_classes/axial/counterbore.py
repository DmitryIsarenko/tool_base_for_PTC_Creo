...
class CounterBore(BaseTool):
    ...
    def __init__(self,
        ...
        self.point_angle = self.get_point_angle()
        self.point_diameter = self.get_point_diameter()
        self.tip_offset = self.get_tip_offset()
        self.set_tool_xml()

    def get_point_angle(self):
        return 180.0
        # return self.catalog_tool_geometry[self.tool_size_from_geom_catalogue]["point_angle"]

    def get_point_diameter(self):
        return self.catalog_tool_geometry[self.tool_size_from_geom_catalogue]["point_diameter"]

    def get_tip_offset(self):
        return self.catalog_tool_geometry[self.tool_size_from_geom_catalogue]["point_len"]

    def get_side_angle(self):
        return self.catalog_tool_geometry[self.tool_size_from_geom_catalogue]["side_angle"]

    def calc_len_out_of_holder(self):
        tail_holding_len = self.catalog_tool_geometry[self.tool_size_from_geom_catalogue]["tail_for_hold"]
        full_body_len = self.catalog_tool_geometry[self.tool_size_from_geom_catalogue]["full_body_len"]
        return full_body_len - tail_holding_len

    def get_tool_flute_length(self) -> float:
        return self.catalog_tool_geometry[self.tool_size_from_geom_catalogue]["flute_len"]



    # def get_tool_teeth_num(self):
    #     return self.catalog_tool_geometry[self.tool_size_from_geom_catalogue]["teeth_num"]
    ...
    def create_tool_name_for_xml(self):
        ...
        m = self.tool_size_from_geom_catalogue
        d1 = self.clear_str_from_trailing_zeros(str(self.get_point_diameter()), sep=".").replace(".", "-")
        d2 = self.clear_str_from_trailing_zeros(str(self.tool_data["CUTTER_DIAM"]), sep=".").replace(".", "-")
    ...
    <ToolingSetup>
        <Tool Id="{self.tool_data["tool_name_for_xml"]}" RefXmlId="encref_1" Type="{self.tool_data["tool_type"]}">
            ...
            <MfgParam Name="TIP_OFFSET" Value="{self.tip_offset}"/>
